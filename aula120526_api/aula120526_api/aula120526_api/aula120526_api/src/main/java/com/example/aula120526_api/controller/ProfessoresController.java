...
import com.example.aula120526_api.service.ProfessoresService;
...
public class ProfessoresController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, Object>> salvar (@Valid @RequestBody ProfessoresRequestDTO professoresRequestDTO){
        service.salvarTodos(professoresRequestDTO);
        return ResponseEntity.status(HttpStatus.CREATED).body(Map.of("Mensagem", "Cadastro realizado com sucesso!✅"));
    }
    @PutMapping("/{id}")
    public ResponseEntity <Map<String, Object>> atualizar (@PathVariable Long id, @RequestBody @Valid ProfessoresRequestDTO professoresRequestDTO){
        service.atualizarCadastro(id, professoresRequestDTO);
